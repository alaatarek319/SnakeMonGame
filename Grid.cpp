...
void Grid::SaveAll(ofstream& outfile, int type) {
	...
	if (type == 1)
	{
		outfile << Ladder::GetLadderCount()<< endl;
	}
	if (type == 2) {

		outfile << Snake::GetSnakeCount() << endl;
	}
	if (type == 3) {

		outfile << Card::GetCardCount() << endl;
	}
		for (int i = NumVerticalCells - 1; i >= 0; i--) // bottom up
		{
			for (int j = 0; j < NumHorizontalCells; j++) // left to right
	...
	for (int i = NumVerticalCells - 1; i >= 0; i--) // bottom up
	{
		for (int j = 0; j < NumHorizontalCells; j++) // left to right
		{
			...
			if (CellList[i][j]->HasCard()) {
				...
				Card * ResetCard=dynamic_cast<Card *>(CellList[i][j]->GetGameObject());
...
GameObject* Grid::GetGameObj(const CellPosition& Cell)
{
	for (int i = NumVerticalCells - 1; i >= 0; i--)
		for (int j = 0; j < NumHorizontalCells; j++)
			if (CellList[i][j]->GetGameObject() && Cell.HCell() == j && Cell.VCell() == i)
...
bool Grid::IsOverLapping(GameObject* newObj)
{
	...
	for (int i = NumVerticalCells - 1; i >= 0; i--) // bottom up
		for (int j = 0; j < NumHorizontalCells; j++) // left to right
			if (CellList[i][j]->GetGameObject())
			{
				found = CellList[i][j]->GetGameObject()->IsOverLapping(newObj);
				if (found)
					return found;
			}
	return false;
