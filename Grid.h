...
public:
	...
	void ResetCards();                  //It loops around the Cards in order to reset them

	GameObject* GetGameObj(const CellPosition& Cell); //Getter for the game object in the grid

	bool IsOverLapping(GameObject* newObj);           //checks if the Snake or ladder is overlapping with one of the objects in the cell list

	void SaveAll(ofstream& outfile, int type);        //It Loops around the objects int the cell list in order to save them
